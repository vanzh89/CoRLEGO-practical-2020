...
%randperm()
imageNames={'RRRG.png','GRRR.png','RGGG.png','BRRR.png','BBBG.png','RRRB.png'};%1-2 f-repeat %1-3 f-swap % 1-4 p-rep-D %2-5 p-rep-T %3-5 p-swap-DT %4-5p-swap-TD

fieldSize=[30 , 30];
currentSelection = 4; %
currentOrder=[6 3 5 4];
...
sim.addElement(SingleNodeDynamics('nodeRed', 40, 12, 4, -10, 0.2, samplingRange, samplingResolution), 'targetImage','inputForRed');
sim.addElement(SingleNodeDynamics('nodeGreen', 40, 12, 4, -10, 0.2, samplingRange, samplingResolution), 'targetImage','inputForGreen');
sim.addElement(SingleNodeDynamics('nodeBlue', 40, 12, 4, -10, 0.2, samplingRange, samplingResolution), 'targetImage','inputForBlue');

sim.addElement(PreprocessingRGB('preprocessing'));
sim.addConnection('targetImage','imageRed','preprocessing');
sim.addConnection('targetImage','imageGreen','preprocessing');
sim.addConnection('targetImage','imageBlue','preprocessing');
sim.addConnection('nodeRed','output','preprocessing');
sim.addConnection('nodeGreen','output','preprocessing');
sim.addConnection('nodeBlue','output','preprocessing');

threshold=-5;
sim.addElement(NeuralField('targetLocationMap', fieldSize, 40, threshold*6, 4));
sim.addConnection('preprocessing','output','targetLocationMap');



sim.addElement(ModifiedHand2D('hand', fieldSize, 6,6, 7, fieldSize(:,1)/2, fieldSize(:,2)/2));

sim.addElement(ModifiedGaussStimulus2D('fixedStimuli', fieldSize, 6, 6, 7, fieldSize(:,1)/2, fieldSize(:,2)/2));
sim.addElement(ModifiedConvolution('handTargetDifferenceMap', fieldSize , 1 ,0, 5,'same'), {'hand', 'targetLocationMap'},{'output','output'});

% add d field

sim.addElement(NeuralField('field d', fieldSize, 20, threshold, 4),{'fixedStimuli','handTargetDifferenceMap'},{'output','output'});
sim.addElement(ModifiedNeuralField('velocityMap', fieldSize, 5, -0.2, 4));
...
sim.addElement(ScaleInput('c_31', [1, 1]), 'nodeRed', 'output', 'nodeBlue');
sim.addElement(ScaleInput('c_13', [1, 1]), 'nodeBlue', 'output', 'nodeRed');
sim.addElement(ScaleInput('c_23', [1, 1]), 'nodeBlue', 'output', 'nodeGreen');
sim.addElement(ScaleInput('c_32', [1, 1]), 'nodeGreen', 'output', 'nodeBlue');
...
sim.addElement(RunningHistory('historyBlueNodeActivation', [1, 1], historyDuration, 1), 'nodeBlue', 'activation');
sim.addElement(RunningHistory('historyBlueNodeOutput', [1, 1], historyDuration, 1), 'nodeBlue', 'output');
...
sim.addElement(SumInputs('shiftedStimulusBlue', [1, 1]), {'targetImage', 'nodeBlue'}, {'inputForBlue', 'h'});
...
sim.addElement(RunningHistory('stimulusHistoryBlue', [1, 1], historyDuration, 1), 'shiftedStimulusBlue');
...
%         'actv.RedNode', 'relative time', 'activation','output'), [1, 2]);
...
%         { {'bo', 'XData', 0, 'MarkerFaceColor', 'b'}, {'Color', [0, 0.5, 0], 'LineWidth', 3, 'XData', 0:-1:-historyDuration+1}, ...
%         {'b-', 'LineWidth', 3, 'XData', 0:-1:-historyDuration+1}, {'r-', 'LineWidth', 3, 'XData', 0:-1:-historyDuration+1}},...
%         'actv.GreenNode', 'relative time', 'activation','output'), [2.1, 2]);
%     
%     gui.addVisualization(MultiPlot({'nodeBlue', 'stimulusHistoryBlue', 'historyBlueNodeActivation','historyBlueNodeOutput'}, {'activation', 'output', 'output','output'}, ...
%         [1, 1, 1, 1], 'horizontal', ...
%         {'FontSize',18,'XLim', [-historyDuration, 10], 'YLim', [-10, 15], 'Box', 'on', 'XGrid', 'on', 'YGrid', 'on'}, ...
%         { {'bo', 'XData', 0, 'MarkerFaceColor', 'b'}, {'Color', [0, 0.5, 0], 'LineWidth', 3, 'XData', 0:-1:-historyDuration+1}, ...
%         {'b-', 'LineWidth', 3, 'XData', 0:-1:-historyDuration+1}, {'r-', 'LineWidth', 3, 'XData', 0:-1:-historyDuration+1}},...
%         'actv.BlueNode', 'relative time', 'activation','output'), [3.2, 2]);
% 
...
% 
%     gui.addVisualization(ExtendedRGBImage('targetImage', 'image','hand','positionX','positionY', {'FontSize',20,'XTick', [], 'YTick', []}, {},{'LineWidth',4},'\fontsize{20}Hand Trajectory'), [4, 1], [1.5, 1]);
...
%     gui.addControl(ParameterSlider('h_Red', 'nodeRed', 'h', [-10, 20], '%0.1f', 1, 'resting level of node u_1'), [1, 1],[1.2,1]);
...
% 
%     gui.addControl(ParameterSlider('h_Green', 'nodeGreen', 'h', [-10, 20], '%0.1f', 1, 'resting level of node u'), [8, 1]);
...
%%
% ManyPeaksInVmap=0;
setSize=4; %6 different target image
numberOfSet=1; %it is changeable

historyOfOrder=zeros(numberOfSet,setSize);
breakImage=6; %7th image is a black image
rateConstant=0.01;
trialTime=1250;
breakTime=1;
...
%%
% % 
% reorederedHistory=[1 2 1 2];
reorederedHistory = currentOrder;
for i=2:numberOfSet
...
for sS= 1:numberOfTrial
    figure(4), maxsubplot(1,numberOfTrial,5-sS),imshow(imageNames{1,reorederedHistory(sS)}); hold on, plot(handPositionX(:,sS),handPositionY(:,sS),'*'); hold off;
%     subplot(numberOfTrial,2,2*sS), plot(handVelocity(:,sS));
end
% 
mean(movementTime(2:4))
std(movementTime(2:4))